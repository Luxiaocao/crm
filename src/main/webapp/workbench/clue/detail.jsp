	...
	$(function(){
		...
		$("#aname").keydown(function (event) {
			if(event.keyCode==13){
				...
				//模态窗口回车键会默认清空数据并刷新页面,return false可以取消该行为
				return false;
			}
		})
		$("#qx").click(function(){
		...
		$("#activityRelationTbody").on("click",$("input[name=xz]"),function(){
		...
		$("#bundBtn").click(function(){
			...
			var parms = "clueId=${c.id}";
			$.each($xz,function(i,e){
				parms += "&activityId=" + $xz[i].value;
			})
			$.ajax({
				url : "workbench/clue/bund.do?" + parms,
				type : "post",
				dataType : "json",
				success : function(data){
					if(data.success){
...
<body>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		...
		<div style="position: relative; height: 50px; width: 500px;  top: -72px; left: 700px;">
			<button type="button" class="btn btn-default" onclick="window.location.href='workbench/clue/convert.jsp?id=${c.id}&fullname=${c.fullname}&appellation=${appellation}&company=${c.company}&owner=${c.owner}';"><span class="glyphicon glyphicon-retweet"></span> 转换</button>
